...
export class ProductsService {
  ...
  getAll(): Observable<any> {
    return this.http.get<any>(routeurls.BASE_API_URL + routeurls.PRODUCT_API_BASE_URL).pipe(
      retry(3),
      map(res => {
        console.log(res);
        if (!res) {
  ...
  getbygroup(gr:any): Observable<any> {
    ...
    return this.http.get<any>(routeurls.BASE_API_URL + routeurls.PRODUCT_API_BASE_URL + "/getbygroup", { params: new HttpParams().set('group', gr.toString()) })
    .pipe(
      retry(3),
      map(res => {
        console.log(res);
